...
<body>
    @php
        $viewLabel = ['doctor' => 'theo bác sĩ', 'month' => 'theo tháng', 'matrix' => 'ma trận'][$view] ?? 'theo bác sĩ';
    @endphp
    <h1>Báo cáo tiền lương toàn bộ bác sĩ - Năm {{ $year }}</h1>
    <div class="muted">Chế độ: {{ $viewLabel }} · Xuất lúc {{ now('Asia/Ho_Chi_Minh')->format('H:i d/m/Y') }}
    ...
    <table class="kpis">
        <tr>
            <td><div class="label">Tổng quỹ lương năm (đã chốt)</div><div class="value">{{ number_format($summary['total_payroll_official'], 0, ',', '.') }} đ</div></td>
            <td><div class="label">Bác sĩ có lương</div><div class="value">{{ $summary['doctors_with_salary'] }}/{{ $summary['doctors_total'] }}</div></td>
            <td><div class="label">Tổng phiếu</div><div class="value">{{ $summary['total_slips'] }}</div></td>
        ...
        <tr>
            <td><div class="label">B.sĩ/tháng chưa lập</div><div class="value">{{ $summary['doctor_months_not_created'] }}</div></td>
