...
// A5/UI11 - xuat bao cao luong nam theo bo loc hien tai.
const AnnualExportDialog = ({ open, params, onClose }) => {
  ...
  const submit = async () => {
    setBusy(true);
    try {
      const res = await salaryAnnualReportApi.export({ ...params, format });
      const blob = new Blob([res.data], { type: MIME[format] });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `bao-cao-luong-nam-${params.year}.${format}`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4" onClick={onClose}>
      <div className="w-full max-w-[440px] rounded-xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <h3 className="text-[14px] font-extrabold text-slate-900">Xuất báo cáo lương năm {params.year}</h3>
...
export default AnnualExportDialog;
