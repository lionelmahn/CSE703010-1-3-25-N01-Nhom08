...
const ServiceDetailPanel = ({
  ...
  const handleStatusChange = async (status, reason) => {
    const response = await serviceCatalogApi.changeStatus(serviceId, { status, reason });
    (response.data?.warnings || []).forEach((warning) => {
      toast({
        title: warning.code ? `Cảnh báo ${warning.code}` : 'Cảnh báo',
        description: warning.message || 'Dịch vụ đã cập nhật, nhưng cần kiểm tra thêm cấu hình vận hành.',
      });
    });
