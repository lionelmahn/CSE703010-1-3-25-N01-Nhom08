...
/**
 * UC5 - phan trang danh sach ho so benh nhan.
 */
const Pagination = ({ meta, page, onPageChange, perPage, onPerPageChange }) => {
  const lastPage = Math.max(1, Number(meta?.last_page || 1));
  const total = Number(meta?.total || 0);
  const realPerPage = Number(meta?.per_page || perPage);
  const from = total === 0 ? 0 : (page - 1) * realPerPage + 1;
  const to = Math.min(page * realPerPage, total);
  ...
  return (
    <div className="p-2 border-t flex flex-col sm:flex-row gap-2 justify-between items-center text-xs text-gray-500 bg-white">
      <span>
        Hiển thị <strong>{from}</strong>–<strong>{to}</strong> trong tổng số <strong>{total}</strong> hồ sơ
      </span>
