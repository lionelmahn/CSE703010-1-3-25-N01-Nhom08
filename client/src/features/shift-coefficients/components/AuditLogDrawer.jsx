...
import { actionLabel, formatDateTime } from '../utils';
...
const AuditLogDrawer = ({ open, coefficient, onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const { data } = await shiftCoefficientApi.auditLogs(coefficient.id, filters);
        setLogs(data || []);
      } catch (err) {
        setError(err?.response?.data?.message || 'Không tải được nhật ký thao tác.');
        setLogs([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/45">
      <section className="h-full w-full max-w-[760px] overflow-y-auto bg-white shadow-2xl">
        <div className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-slate-200 bg-white px-5">
          <h3 className="text-[14px] font-extrabold text-slate-950">Nhật ký thao tác - {coefficient.code}</h3>
        ...
        <div className="space-y-4 p-5 text-[12px]">
          <div className="grid gap-2 md:grid-cols-4">
            <select value={filters.action} onChange={(event) => updateFilter('action', event.target.value)} className="h-9 rounded-md border border-slate-200 px-2">
              <option value="all">Tất cả hành động</option>
              {Object.entries(ACTION_LABEL).map(([action, label]) => (
                <option key={action} value={action}>{label}</option>
              ))}
            </select>
            <input value={filters.actor} onChange={(event) => updateFilter('actor', event.target.value)} placeholder="Người thao tác" className="h-9 rounded-md border border-slate-200 px-2" />
          ...
          <div className="overflow-x-auto rounded-md border border-slate-200">
            <table className="min-w-[680px] w-full text-[11px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading && <tr><td colSpan={4} className="px-3 py-6 text-center text-slate-500">Đang tải...</td></tr>}
                {!loading && logs.length === 0 && <tr><td colSpan={4} className="px-3 py-6 text-center text-slate-500">Chưa có nhật ký thao tác.</td></tr>}
                {!loading && logs.map((log) => (
                  <tr key={log.id}>
                    <td className="px-3 py-2">{formatDateTime(log.created_at)}</td>
                    <td className="px-3 py-2">{log.actor_name || 'Hệ thống'}</td>
                    <td className="px-3 py-2 font-semibold">{actionLabel(log.action)}</td>
                    <td className="px-3 py-2 text-slate-600">
                      {log.details?.coefficient ? `Hệ số x${log.details.coefficient}` : log.details?.reason || log.details?.new_effective_to || '-'}
                    </td>
                  </tr>
                ))}
              </tbody>
